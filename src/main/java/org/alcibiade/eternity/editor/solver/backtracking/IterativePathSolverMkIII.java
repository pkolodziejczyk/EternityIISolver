...
public class IterativePathSolverMkIII extends EternitySolver implements ClusterListener {
	...
	public String getSolverName() {
		return "Iterative Path Solver MkIII $Revision: 260 $";
	}
	...
	private void updateRecordData(int destOffset) {
		if (destOffset > record) {
			...
			int score = solutionGrid.countPairs();

			for (QuadModel quad : solutionGrid) {
				if (!quad.isClear()) {
					quad.setLocked(true);
				}
			}
			...
			clusterManager.submitSolution(solutionGrid, score);
			record = destOffset;
		}
	}
