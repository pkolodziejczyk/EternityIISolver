...
import org.alcibiade.eternity.editor.model.QuadMatchingComparator;
...
public class IterativePathSolverMkIV extends EternitySolver implements ClusterListener {
	...
	public IterativePathSolverMkIV(GridModel grid, GridModel solutionGrid,
	...
	public String getSolverName() {
		return "Iterative Path Solver MkIV $Revision: 266 $";
	}
	...
	private boolean explore(int destOffset, int destIndex) {
		...
		List<QuadModel> possibleQuads = new ArrayList<QuadModel>();
		...
		for (int srcIndex = 0; !result && srcIndex < positions; srcIndex++) {
			...
			possibleQuads.add(src);
		}

		Collections.sort(possibleQuads, new QuadMatchingComparator(missing, true));

		for (QuadModel src : possibleQuads) {
			src.copyTo(dest);
			src.clear();
