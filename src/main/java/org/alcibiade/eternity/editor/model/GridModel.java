...
public class GridModel extends AbstractQuadGrid implements Cloneable {
	...
	public void sort(QuadModel reference) {
		Comparator<QuadModel> comparator = new QuadMatchingComparator(reference);
		sort(comparator);
	}

	public void sort(Comparator<QuadModel> comparator) {
		boolean updated = true;

		while (updated) {
			updated = false;

			for (int i = 0; i < gridQuads.size() - 1; i++) {
				QuadModel a = gridQuads.get(i);
				QuadModel b = gridQuads.get(i + 1);
